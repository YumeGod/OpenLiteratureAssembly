...
import just.monika.主播为什么开我端子.modules.render.ViewClip;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if (movingobjectposition != null)
                    {
                        ...
                        if (d7 < d3 && (!ModuleManager.getModule(ViewClip.class).enabled))
                        {
                            d3 = d7;
                        }
                    }
     ...
     * Render player hand
     */
    public void renderHand(float partialTicks, int xOffset)
    {
        if (!this.debugView)
        {
            GlStateManager.matrixMode(5889);
        ...
        if (f5 > 0.0F)
        {
            ...
            for (int i1 = k - b0; i1 <= k + b0; ++i1)
            {
                for (int j1 = i - b0; j1 <= i + b0; ++j1)
                {
                    ...
                    blockposmutableblockpos.func_181079_c(j1, 0, i1);
                    BiomeGenBase biomegenbase = worldclient.getBiomeGenForCoords(blockposmutableblockpos);

                    if (biomegenbase.canSpawnLightningBolt() || biomegenbase.getEnableSnow())
                    {
                        int l1 = worldclient.getPrecipitationHeight(blockposmutableblockpos).getY();
                        int i2 = j - b0;
                        int j2 = j + b0;
                        ...
                        if (i2 != j2)
                        {
                            ...
                            blockposmutableblockpos.func_181079_c(j1, i2, i1);
                            float f1 = biomegenbase.getFloatTemperature(blockposmutableblockpos);
                            ...
                            if (worldclient.getWorldChunkManager().getTemperatureAtHeight(f1, l1) >= 0.15F)
                            {
                                ...
                                blockposmutableblockpos.func_181079_c(j1, k2, i1);
                                int l2 = worldclient.getCombinedLight(blockposmutableblockpos, 0);
                                int i3 = l2 >> 16 & 65535;
                                int j3 = l2 & 65535;
                            ...
                            else
                            {
                                ...
                                blockposmutableblockpos.func_181079_c(j1, k2, i1);
                                int k3 = (worldclient.getCombinedLight(blockposmutableblockpos, 0) * 3 + 15728880) / 4;
                                int l3 = k3 >> 16 & 65535;
                                int i4 = k3 & 65535;
